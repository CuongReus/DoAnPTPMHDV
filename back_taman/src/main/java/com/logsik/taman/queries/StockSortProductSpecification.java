...
public class StockSortProductSpecification implements Specification<Stock> {
	...
	private Float minInStock = 0.0F;
	private Float maxInStock = 2000000000.0F; // 2 billions

	public StockSortProductSpecification(String productName, String productCategoryId,String supplierId, List<String> productUnit,
			List<String> productSize, Float minInStock, Float maxInStock) {
		super();
		...
		this.setMinInStock(minInStock);
		this.setMaxInStock(maxInStock);
	}
	...
	public Predicate toPredicate(Root<Stock> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		...
		if (!StringUtils.isEmpty(getProductName())) {
			...
			query.having(builder.greaterThanOrEqualTo(builder.sumAsDouble(quantityExp), Double.valueOf(minInStock)),
						builder.lessThanOrEqualTo(builder.sumAsDouble(quantityExp), Double.valueOf(maxInStock)));
			query.orderBy(builder.desc(builder.sumAsDouble(quantityExp)));
	...
	public Float getMaxInStock() {
		return maxInStock;
	}

	public void setMaxInStock(Float maxInStock) {
