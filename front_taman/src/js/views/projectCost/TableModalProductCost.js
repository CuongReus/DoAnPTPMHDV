...
const ProductCost = (props) => {
    const { t, handleShowModal, deleteProjectCost,handleShowApprovalAllModal, currentUser,projectCostItem, handleAddNewPayment, handleShowProductPayment, reloadProjectCost, dataProjectCostPayment, keyItem } = props;
    if (dataProjectCostPayment && dataProjectCostPayment.isShown) {
        var totalPercentPaid = 0;
        var productPaymentRows = dataProjectCostPayment.listPayments.map(productItem => {
            totalPercentPaid += productItem.percentPaid;
            return <ProjectCostPayment key={"VAT_TU" + productItem.id} reloadProjectCost={reloadProjectCost} totalPercentPaid={totalPercentPaid} projectCostDto={projectCostItem} paymentType="VAT_TU" projectCostPaymentDto={productItem}  ></ProjectCostPayment>
        })
        var productPaymentHeader =
            [<tr className="success" style={{ textAlign: 'center' }}>
                <td colSpan="2"></td>
                <td>Số Đợt</td>
            ...
            </tr>].concat(productPaymentRows)
    }
    var body = [<tr key={"productCost_" + projectCostItem.id}>
        <td>
            {/* <SecuredComponent allowedPermission="admin.projectDetail.read"> */}
            {dataProjectCostPayment && dataProjectCostPayment.isShown ? null : <button className="bg-info icon-plus22 " onClick={() => handleShowProductPayment(projectCostItem.id)}></button>}
            {dataProjectCostPayment && dataProjectCostPayment.isShown ? <button className="bg-info icon-dash" onClick={() => handleShowProductPayment(projectCostItem.id)}></button> : null}
            {/* </SecuredComponent> */}
        </td>
            ...
            <ul className="icons-list">
                <li className="dropdown">
                    ...
                    <ul className="dropdown-menu dropdown-menu-right">
                        ...
                        {SecurityUtils.hasPermission(currentUser, "admin.projectCost.productC&U")  ?
                        <li><a onClick={() => handleShowModal(projectCostItem.id)}><i className="icon-pencil"></i>Chỉnh Sửa</a></li>:null
                        }
                        <SecuredComponent allowedPermission="admin.projectCostApproval.productAllowApproval">
                        <li><a onClick={() => handleShowApprovalAllModal(projectCostItem.id)}><i className="icon-stack-check"></i>Duyệt Tất Cả</a></li>
                        </SecuredComponent>
                        {SecurityUtils.hasPermission(currentUser, "admin.projectCost.productDelete") ?
    ...
    </tr>].concat(productPaymentHeader)
    return [body];
...
class TableModalProductCost extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            listProjectCost: [],
        ...
        this.handleShowProductPayment = this.handleShowProductPayment.bind(this);
    ...
    handleShowProductPayment(projectCostId) {
        var listPayment = this.state.listProjectCostPayment;
        var reloadNum = this.state.reloadNum;
        if (projectCostId) {
    ...
    render() {
        ...
        const { t, handleSubmit, submitting,currentUser, title, invalid, createdUser, lastedUpdateUser } = this.props;
        ...
        var rows = dataProjectCost.map(projectCostItem => {
            return (
                <ProductCost
                    ...
                    handleShowProductPayment={this.handleShowProductPayment}
        ...
        newModal =
            <div style={{ width: '100%' }}>
                ...
                <div style={{ width: '1150px', marginLeft: "-40px", fontSize: "12px" }} className="modal-content">
                                ...
                                <table className="table table-togglable table-bordered table-hover">
                                    <thead>
                                        <tr className="bg-green">
                                            ...
                                            <th data-toggle="true">Chủng Loại Vật Tư</th>
                    ...
                    {this.state.isShowModalPaymentCost ? <ModalProductPayment title="Đợt Thanh Vật Tư" dataProjectCostPayment={dataProjectCostPayment.length != 0 ? dataProjectCostPayment : null} projectCostDto={this.state.projectCostDto} currentDate={this.state.currentDate} idPayment={null} projectDetailDto={projectDetailDto} show={this.state.isShowModalPaymentCost} onHide={this.handleHidemodal} /> : null}
                    {this.state.isShownProjectCostModal ? <ModalProductCost title="Thanh Toán Vật Tư" lotNumber={dataProjectCost.length + 1} idProjectCost={this.state.idProjectCost} projectDetailDto={projectDetailDto} show={this.state.isShownProjectCostModal} onHide={this.handleHidemodal} /> : null}
...
export default translate('translations')(connect(
    mapStateToProps, mapDispatchToProps)(TableModalProductCost));
