...
import ModalConstructionTeamCost from './ModalConstructionTeamCost';
...
const ConstructionTeamCost = (props) => {
    ...
    if (dataProjectCostPayment && dataProjectCostPayment.isShown) {
        ...
        var otherPaymentRows = dataProjectCostPayment.listPayments.map(constructionTeamItem => {
            totalPercentPaid += constructionTeamItem.percentPaid;
            return <ProjectCostPayment key={"DOI_THI_CONG" + constructionTeamItem.id} reloadProjectCost={reloadProjectCost} totalPercentPaid={totalPercentPaid} projectCostDto={projectCostItem} paymentType="DOI_THI_CONG" projectCostPaymentDto={constructionTeamItem}  ></ProjectCostPayment>
        })
    ...
    var body = [<tr key={"constructionTeamCost" + projectCostItem.id}>
        <td>
            ...
            <ul className="icons-list">
                <li className="dropdown">
                    ...
                    <ul className="dropdown-menu dropdown-menu-right">
                                                {SecurityUtils.hasPermission(currentUser, "admin.projectCost.constructionTeamC&U") ?
                        <li><a onClick={() => handleShowModal(projectCostItem.id)}><i className="icon-pencil"></i>Chỉnh Sửa</a></li>:null
                                                }
                        <SecuredComponent allowedPermission="admin.projectCostApproval.constructionTeamAllowApproval">
                        <li><a onClick={() => handleShowApprovalAllModal(projectCostItem.id)}><i className="icon-stack-check"></i>Duyệt Tất Cả</a></li>
                        </SecuredComponent>
                                                {SecurityUtils.hasPermission(currentUser, "admin.projectCost.constructionTeamDelete") ?
...
class TableModalConstructionTeamCost extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            listProjectCost: [],
    ...
    render() {
        ...
        const { t, handleSubmit, submitting, title,currentUser, invalid, createdUser, lastedUpdateUser } = this.props;
        ...
        newModal =
            <div style={{ width: '100%' }}>
                ...
                <div style={{ width: '1150px', marginLeft: "-40px", fontSize: "12px" }} className="modal-content">
                                ...
                                <table className="table table-togglable table-bordered table-hover">
                                    <thead>
                                        <tr className="bg-green">
                                            ...
                                            <th data-toggle="true">Tên Đội Thi Công</th>
                    ...
                    {this.state.isShowModalPaymentCost ? <ModalConstructionTeamPayment title="Đợt Thanh Toán Đội Thi Công" dataProjectCostPayment={dataProjectCostPayment.length != 0 ? dataProjectCostPayment : null} projectCostDto={this.state.projectCostDto} currentDate={this.state.currentDate} idPayment={null} projectDetailDto={projectDetailDto} show={this.state.isShowModalPaymentCost} onHide={this.handleHidemodal} /> : null}
                    {this.state.isShownProjectCostModal ? <ModalConstructionTeamCost title="Thanh Toán Đội Thi Công" lotNumber={dataProjectCost.length + 1} idProjectCost={this.state.idProjectCost} projectDetailDto={projectDetailDto} show={this.state.isShownProjectCostModal} onHide={this.handleHidemodal} /> : null}
...
export default translate('translations')(connect(
    mapStateToProps, mapDispatchToProps)(TableModalConstructionTeamCost));
