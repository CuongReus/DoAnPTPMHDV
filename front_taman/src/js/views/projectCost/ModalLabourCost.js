...
import { RenderInputWithDiv, RenderInputWithGen, RenderTextArea, RenderDatePicker, RenderCheckbox, RenderSelect, RenderNumberInput, RenderMoneyFormat, RenderDatePickerMinPrev, RenderMultiSelect } from '../../components/formInputs';
...
const validate = values => {
   ...
   if(values.startWorkDate){
    if(parseInt(moment(values.startWorkDate).format("MM")) != parseInt(values.currentMonthLink)){
        errors.startWorkDate = values.currentMonthLink? "Đây là đợt thanh toán cho tháng "+ values.currentMonthLink  +" vui lòng chọn lại móc thời gian" : null
    }
    if(parseInt(moment(values.startWorkDate).format("YYYY")) !=  parseInt(values.currentYearLink)){
        errors.startWorkDate =values.currentYearLink ? "Đây là đợt thanh toán cho năm "+ values.currentYearLink  +" vui lòng chọn lại móc thời gian" : null 
    }
}

    if(values.endWorkDate){
        if(parseInt(moment(values.endWorkDate).format("MM")) != parseInt(values.currentMonthLink)  ){
            errors.endWorkDate = values.currentMonthLink?  "Đây là đợt thanh toán cho tháng "+ values.currentMonthLink +" vui lòng chọn lại móc thời gian" : null
        }
        if(parseInt(moment(values.endWorkDate).format("YYYY")) !=  parseInt(values.currentYearLink)){
            errors.endWorkDate = values.currentYearLink  ? "Đây là đợt thanh toán cho năm "+ values.currentYearLink +" vui lòng chọn lại móc thời gian" : null
        }
    }
    return errors;
}

const selector = formValueSelector('ModalLabourCost');
...
const mapStateToProps = state => {
    ...
    var updateValue = {
        ...
        startWorkDate:state.projectCostReducer.updatingProjectCost && state.projectCostReducer.updatingProjectCost.startWorkDate ? moment(state.projectCostReducer.updatingProjectCost.startWorkDate) : null,
        endWorkDate:state.projectCostReducer.updatingProjectCost && state.projectCostReducer.updatingProjectCost.endWorkDate ? moment(state.projectCostReducer.updatingProjectCost.endWorkDate) : null,
        notifyTo: null,
        notifyMessage: null,
        ...
        // month: state.projectCostReducer.updatingProjectCost && state.projectCostReducer.updatingProjectCost.month ? state.projectCostReducer.updatingProjectCost.month : moment(today).format("M"),
        // year : state.projectCostReducer.updatingProjectCost && state.projectCostReducer.updatingProjectCost.year ? state.projectCostReducer.updatingProjectCost.year : moment(today).format("YYYY"),
    ...
    return {
        ...
        projectDetailId: selector(state, "projectDetailId"),
...
const mapDispatchToProps = dispatch => ({
    loadProjectCost: (payload) =>
        ...
        dispatch({
            meta: { form: "ModalLabourCost", field: fieldName },
            payload: value,
...
class ModalLabourCost extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            listAllUsers:[],
            listAllLabourByProjectDetailId:[],
    ...
    componentWillMount() {
        const { loadProjectCost, loadProjectDetailDto, projectDetailDto,updateField,lotNumber,currentDate} = this.props;
        ...
        setTimeout(() => {
            updateField("currentMonthLink",moment(currentDate).format("MM"))
            updateField("currentYearLink",moment(currentDate).format("YYYY"))
            }, 50); 
    ...
    handleAdd(values) {
        ...
        const {currentUser,currentDate} = this.props;
        var today = moment(new Date,"DD/MM/YYYY");
        var id = this.props.idProjectCost;
        var url = '/projectCost/add';
        var bodyObject = {
            projectDetailId:values.projectDetailId,
            month: values.month?values.month  :moment(currentDate).format("M"),
            year: values.year?values.year:moment(currentDate).format("YYYY"),
    ...
    render() {
        ...
        const { handleSubmit,status, submitting, title, invalid,dataProjectCost, createdUserId,lastedUpdateUserId,currentUser,initialValues,projectDetailDto,month,year } = this.props;
        ...
        const dataLabourByProjectDetailId = this.state.listAllLabourByProjectDetailId;
        var id = this.props.idProjectCost;
        
        var optionLabourByProjectDetailId = [];
        if(dataLabourByProjectDetailId){
            dataLabourByProjectDetailId.map(item=>{
                optionLabourByProjectDetailId.push({label:item.fullName,value:item.labourId });
        ...
        //end Date of lot number for labour
        var minDate = null;
        // numberAddDay use for component DatePickerMinPrev
        var numberAddDay = 0;
        if(dataProjectCost && !id){
            minDate =  moment(dataProjectCost[dataProjectCost.length -1].endWorkDate).format("DD/MM/YYYY");
            numberAddDay = 1;
            // endDateByLastedLot = 
        }else if(projectDetailDto && projectDetailDto.efficiency){
            minDate = moment(projectDetailDto.efficiency.startActualProgressDate).format("DD/MM/YYYY");
        ...
        dataPersonels.map(item => {
            // if(SecurityUtils.hasPermission(item, "admin.holiday.approvalLetter")){
               if(SecurityUtils.hasPermission(item, "admin.projectCostApproval.labourAllowApproval")){
                    ...
                    <Modal.Body>
                        {submitting ? <LoadingScreen /> :
                            <form className="form-horizontal" role="form" onSubmit={handleSubmit(this.handleAdd)}>
                                ...
                                {/* <Field name="month" label="Tháng" placeholder="Nhập tháng để lọc nhân công..."  component={RenderNumberInput} ></Field>
                                <Field name="year" label="Năm" placeholder="Nhập tháng để lọc nhân công..."  component={RenderNumberInput} ></Field> */}
                                <Field name="title" label="Nội Dung" placeholder="Nhập nội dung..." component={RenderInputWithDiv}></Field>
                                ...
                                <Field name="startWorkDate" label="Ngày Bắt Đầu Công Việc"dateFormat="DD/MM/YYYY" minDate ={minDate} numberDayAdd= {numberAddDay}   component={RenderDatePickerMinPrev}></Field>
                                <Field name="endWorkDate" label="Ngày Kết Thúc Công Việc" dateFormat="DD/MM/YYYY" component={RenderDatePickerMinPrev}></Field>
                                ...
                                <Field type="hidden" name="totalMoney" placeholder="" thousandSeparator={true} component={RenderMoneyFormat}></Field>
                                <Field type="hidden" name="totalPaid"  placeholder="" thousandSeparator={true} component={RenderMoneyFormat}></Field>
                                ...
                                <Field disabled={SecurityUtils.hasPermission(currentUser, "admin.projectCostApproval.labourAllowApproval")? false:true} name="status" label="Trạng Thái" placeholder="Chọn trạng thái..." options={optionsStatus} component={RenderSelect}></Field>
                              ...
                              <Field name="closeDate" label="Ngày Đóng Thanh Toán"  dateFormat="DD/MM/YYYY" component={RenderDatePicker}></Field>]: null}
                                ...
                                <Field type="hidden" name="currentMonthLink"  component={RenderInputWithDiv}></Field>
                                <Field type="hidden" name="currentYearLink"  component={RenderInputWithDiv}></Field>
