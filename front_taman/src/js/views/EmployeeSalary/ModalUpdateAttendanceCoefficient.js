...
import {LOAD_UPDATING_DEPARTMENT } from '../department/action-types';
...
class ModalUpdateAttendanceCoefficient extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            objDepartment: null
    ...
    handleAdd(values) {
        ...
        var id = this.props.idDepartment;
       var url =  '/employeeSalary/addNewAttendanceCoefficient?month='+month+'&year='+year;
        var bodyObject = {
            code: values.code,
            name: values.name,
            attendanceCoefficient: values.attendanceCoefficient,
            month:month,
            year:year,
            note: values.note
        };
        if (id) {
        var url = '/employeeSalary/updateAttendanceCoefficient?month='+month+'&year='+year;
            bodyObject.id = id;
        }
    ...
    render() {
        ...
        const id = this.props.idDepartment;
        var newModal = null;
            newModal =
                <div style={{ width: '30%' }}>
                    <Modal
                    ...
                    >
                        <Modal.Header closeButton>
                            <Modal.Title id="contained-modal-title-sm"><center>{id ? title : "Thêm mới hệ số ngày công theo phòng ban" }</center></Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
...
export default translate('translations')(connect(
    mapStateToProps, mapDispatchToProps)(
        reduxForm({
            form: 'ModalUpdateAttendanceCoefficient',
            destroyOnUnmount: true,
            enableReinitialize: true,
            validate
        })(ModalUpdateAttendanceCoefficient)));
